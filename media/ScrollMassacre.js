class ScrollMassacre {
  ...
  load() {
    return Promise.all(this.demoManager.loadResource(['army_16x16.png', 'bold_64x32.png', 'thin_32x32.png', 'megadeath-blue_128x128.png', 'Astaroth_3.ym'])).then(data => {
      [this.tinyFont, this.roundFont, this.thinFont, this.bigFont, this.preload] = data;
    });
  }
  ...
  start() {
    return new Promise(endCallback => {
      ...
      this.zik.LoadAndRun(this.demoManager.basepath + 'Astaroth_3.ym');
  ...
  greenScroller(){
    ...
    for(let ys=0,yd=398; ys<106; ys++,yd--)
      this.can.contex.drawImage(this.greenCanDist.canvas, 0,ys,640,1, 0,yd,640,1);
  }
