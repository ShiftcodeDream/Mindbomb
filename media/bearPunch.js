class bearPunch {
  ...
  load(){
    return Promise.all(loadResource([
      'mind-logo1.png','tcb-1.png','punch.png','tlb-logo1.png','slogan.png','laugh.ogg','boing.mp3'
    ])).then(data => 
      [this.mind, this.tcb, this.punch, this.tlb, this.slogan, this.laugh, this.boing] = data
    );
  }
  ...
  start(){
    return new Promise(endCallback => {
      ...
      const me = this;
      this.timeouts.push(setTimeout(()=>{me.punchPos = 434}, 1500));
      this.timeouts.push(setTimeout(()=>{me.punchSpeed = 20}, 3000));
